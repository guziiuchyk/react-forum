...
const CreatePost: React.FC = () => {
    ...
    const handleCreate = (): void => {
        axios.post(`http://localhost:8000/api/posts?topic=${encodeURIComponent(topic)}&content=${encodeURIComponent(content)}${tags ?`&tags=${encodeURIComponent(tags.split(" ").join(", "))}` : ""}`, {}, {withCredentials: true}).then((res) => {
